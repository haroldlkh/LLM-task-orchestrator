...
def released_row_ids_for_flush(
    ...
    expected_per_row = work_units_df.group_by("row_id").agg(pl.len().alias("expected_unit_count"))
    success_per_row = joined.filter(pl.col("status") == "success").group_by("row_id").agg(
...
def released_results_df_for_row_ids(all_results_df: pl.DataFrame, released_row_ids: list[str]) -> pl.DataFrame:
...
def build_pair_status_df(
    ...
    for field_name in fields:
        ...
        blocked_exprs.append(
            pl.when(pl.col(status_col).is_null() | (pl.col(status_col) != "success"))
