...
def _debug_base(unit, request, request_result, request_attempt_count, request_group_size, now):
    return {
        "unit_id": unit["unit_id"],
        "row_id": unit["row_id"],
...
def mark_group_transport_failure(group_units, request, request_result, progress_df, request_attempt_count):
    ...
    for unit in group_units:
        ...
        debug_row = _debug_base(unit, request, request_result, request_attempt_count, request_group_size, now)
        debug_row.update({
            "status": request_result["status"],
            "review_flag": True,
...
def mark_group_parse_failure(group_units, request, raw_output, error_type, error_message, progress_df, request_attempt_count, request_result=None):
    now = utc_now_iso()
    progress_rows = []
    debug_rows = []
    request_group_size = len(group_units)
